...
(async () => {
  ...
  assert.equal(f.calls.filter(u=>u.includes('gtag/js?id=G-4J0QLJT1N0')).length,1);
  const gaConfig=await f.page.evaluate(()=>Array.from(window.dataLayer, x=>Array.from(x)).find(x=>x[0]==='config'&&x[1]==='G-4J0QLJT1N0'));
  assert.equal(gaConfig[2].cookie_domain,'auto');
  assert.equal(f.calls.filter(u=>u.includes('gtag/js?')).length,1);
  assert(!f.calls.some(u=>u.includes('G-ZPS6GNGPXD')||u.includes('G-6V5Y3JP1VJ')));
  assert.equal(await f.page.evaluate(()=>Array.from(window.dataLayer).filter(x=>x[0]==='config').length),1);
  assert.equal(await f.page.locator('head script[src="https://www.googletagmanager.com/gtag/js?id=G-4J0QLJT1N0"]').count(),1);
